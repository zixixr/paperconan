...
- `prefilter_flags` (optional list): deterministic flags supporting the prefilter decision, such as axis/index-like labels, percentage/ratio/normalized/model-output context, low cardinality, boundary/floor/ceiling values, fixed-denominator hints, or repeated fill values. Use these with [judgment-rubric.md](judgment-rubric.md) before surfacing `within_col_*` hits.
...
- `col_a_sample` / `col_b_sample` (optional): small value samples from the relevant column(s), used as an evidence peek when the full table is large. `within_col_*` usually has `col_a_sample`; pairwise relation findings may have both. These samples are useful for explaining repeated values, last-two-decimal patterns, and fixed-denominator checks, but they do not replace opening the original table when making a serious claim.
