...
| `integer_diff_shared_fraction` | 两列**共享高精度小数尾**、整数差取多个值 | `178.7615` vs `112.7615`、`169.8687` vs `115.8687` |
...
| `identical_after_rounding` | 多个 cell 舍到 1 位小数后相同但精确值不同 | 同一舍入值下出现 ≥3 种精确值 |
| `missing_last_digits` | 某些末位数字从不出现 | 在 ≥20 个值中，3、7 等末位完全缺失 |
| `many_equal_pairs` | 两个标注为独立的列里大量 byte-identical | 10 对中 9 对完全一致，仅 1 对不同 |
| `row_pair_digit_coupling` | 两行之间高位改变但小数/个位异常保留 | `197.2 → 167.2`、`165.5 → 155.5` 成串出现 |
| `cross_sheet_position_identical` | 两张 sheet 同位置数值完全一样 | 两张表在大量对应位置数值完全一致 |
| `cross_sheet_value_overlap` | 两张表共享大量小数值（不要求同位置） | 两张标注为独立实验的表共享大量小数值，需澄清样本或处理关系 |
| `cross_sheet_decimal_tail_reuse` | 跨 sheet 多个值长小数尾相同、前导数字不同 | `14.70300997 → 6.70300997` 成串出现 |
...
每条 finding 都带 `severity`、文件、sheet、block 行列范围、规则字符串和 evidence。**跨表类（`cross_sheet_*`）优先级最高** —— 既能抓同一文件内两张 sheet，也能抓两个独立文件之间的数值重复。
