...
def _copy_skill_zip_project(tmp_path):
    project = tmp_path / "copied project"
    project.mkdir()
    shutil.copy2(
        ROOT / "build_skill_zip.sh",
        project / "build_skill_zip.sh",
    )
    shutil.copytree(
        SKILL_DIR,
        project / "skills" / "paperconan",
    )
    for relative in WORKED_EXAMPLE_FILES:
        source = ROOT / "examples" / relative
        destination = project / "examples" / relative
...
def test_skill_zip_default_output_remains_in_repository_root(
    tmp_path,
) -> None:
    project = _copy_skill_zip_project(tmp_path)
    caller = tmp_path / "caller"
    caller.mkdir()

    subprocess.run(
        [str(project / "build_skill_zip.sh")],
        cwd=caller,
        check=True,
        text=True,
        capture_output=True,
    ...
    assert (project / "paperconan-skill.zip").is_file()
    assert not (caller / "paperconan-skill.zip").exists()


def test_skill_zip_rejects_output_aliasing_a_source(tmp_path) -> None:
    project = _copy_skill_zip_project(tmp_path)
    source = project / "skills" / "paperconan" / "SKILL.md"
    original = source.read_bytes()

    proc = subprocess.run(
        [str(project / "build_skill_zip.sh"), str(source)],
        cwd=tmp_path,
        text=True,
        capture_output=True,
    )
...
def test_skill_zip_preserves_existing_output_when_source_is_missing(
    tmp_path,
) -> None:
    project = _copy_skill_zip_project(tmp_path)
    ...
    proc = subprocess.run(
        [str(project / "build_skill_zip.sh"), str(output)],
        cwd=tmp_path,
        text=True,
        capture_output=True,
    )
