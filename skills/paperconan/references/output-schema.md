...
- `n_informative` (optional, `constant_ratio` findings): how many of the finding's `n` rows the ratio actually rests on. Rows that are zero on **both** sides are consistent with every ratio, so they are excluded from it; `n` stays the pair's row count so it keeps the meaning the other relation kinds give it. A `constant_ratio` whose `n_informative` is much smaller than its `n` is a sparse pair — the relation is over a subset, and how much of the column that subset is belongs in the write-up.
