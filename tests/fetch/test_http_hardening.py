...
class _StubResp(io.BytesIO):
    ...
    def geturl(self):
        return "https://api.example.org/result"
...
def _real_http_response(body, content_length):
    ...
    response.begin()
    response.geturl = lambda: "https://api.example.org/result"
    return response
...
def test_http_helpers_raise_incomplete_read_for_truncated_real_http_response(
    ...
    monkeypatch.setattr(
        _http, "open_http", lambda _req, timeout=None: response
    )
...
def test_http_helpers_reject_negative_cap_before_network(
    ...
    monkeypatch.setattr(_http, "open_http", unexpected_urlopen)
...
def test_get_text_preserves_positional_max_bytes_api(monkeypatch):
    body = b"plain text"
    response = _StubResp(body)
    monkeypatch.setattr(
        _http, "open_http", lambda _req, timeout=None: response
...
def test_size_error_does_not_expose_url_or_response_body(monkeypatch):
    ...
    url = (
        f"https://example.org/{path_sentinel}"
        f"?token={credential_sentinel}"
    )
