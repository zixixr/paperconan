...
| `PAPERCONAN_CROSS_SHEET_VALUE_BUDGET` | `50000000` | 全 scan 的跨表逻辑 value visit 预算：axis classification 固定每值 4 次，positional/value family 对 A、B 各扫描 1 次，decimal-tail family 对两侧各扫描 1 次；可行 family 在执行前预检，剩余 pair/value work 用线性聚合精确计算，耗尽时记录精确已执行/跳过 visit 于 `cross_sheet_work_limit` |
