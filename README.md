...
> **守住红线**：paperconan 输出的是**可复核的数值模式**，不是研究完整性审查结论。上面的机构处理是**已公开事实**，不是 paperconan 的判定。工具只负责把任何人都能复现的信号定位清楚；是否需要进入正式审查，仍要靠原始数据、作者回应和机构/期刊核实。
>
...
`paperconan` 输出的是 **算法标注的可疑模式**，不是研究完整性审查结论。最终判定需由原作者澄清、期刊编辑部核实，或经独立同行复议。
